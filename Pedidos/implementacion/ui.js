    ...
    .map(
      () => `
        <article class="mod-stat-card">
          <div class="mod-stat-icon skeleton" style="opacity: 0.5"></div>
          <div class="mod-stat-info" style="width: 100%">
            <div class="skeleton skeleton-line skeleton-line--sm" style="width: 60%; margin-bottom: 8px;"></div>
            <div class="skeleton skeleton-line skeleton-line--lg" style="width: 40%"></div>
          </div>
        </article>
      `,
    )
...
export function renderStats(stats, { loading = false } = {}) {
  ...
  return stats
    .map(
      (stat) => `
        <article class="mod-stat-card">
            <div class="mod-stat-icon mod-stat-icon--${stat.tone}">
                <span class="icon-slot" data-icon="${stat.icon}"></span>
            </div>
            <div class="mod-stat-info">
                <span class="mod-stat-label">${escapeHtml(stat.label)}</span>
                <div class="mod-stat-value">${escapeHtml(stat.value)}</div>
                <span class="mod-stat-label" style="font-size: 10px; margin-top: 4px;">${escapeHtml(stat.helper)}</span>
            </div>
        </article>
      `,
    )
